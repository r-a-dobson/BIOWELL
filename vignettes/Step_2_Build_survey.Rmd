...
+ `start_questions_ID` - A character string or vector (equal length to `start_questions`) providing a unique ID for each start question. This is used in the resulting data frame of participant responses to avoid lengthy column names if question text is used. 
...
+ `biowell_situations_ID` - A string or vector (equal length to
...
questions after the BIO-WELL questions.
...
+ `end_questions_ID` - A character string or vector (equal length to `end_questions`)
providing a unique ID for each end question. This is used in the resulting data
...
the same order as `end_questions`). We include five options:
  ...
  4) `likert_five`: participants can move a five-point slider of the Likert.
  scale ("Strongly agree" to "Strongly disagree"). 
  5) `likert_seven`: participants can move a seven-point slider of the Likert scale (includes
  "Somewhat agree" and "Somewhat disagree").
...
+ `offline_mode` - A logical (`TRUE` or `FALSE`) specifying whether to upload input response data to Dropbox. This must be `TRUE` when uploading the package to the Shiny Server or data *will not be saved*. The offline mode is included to prevent data upload to Dropbox during the survey development stage.  
...
+ `language` - An exciting feature of the `BIOWELL` package is the ability to
specify the language used on the BIO-WELL sliders. This can be selected from the
32 languages listed below. We recommend ensuring that your questions are also in
your chosen language.

**Available languages**: 'arabic', 'bengali_india', 'chinese_hong kong', 'chinese_taiwan',
'danish', 'dutch', 'edo', 'english', 'finnish', 'french', 'german', 'gujarati',
'indonesian', 'irish', 'italian', 'japanese', 'korean', 'malay', 'norwegian',
'polish', 'portuguese', 'portuguese_brasil', 'punjabi_india', 'punjabi_pakistan',                                 'russian', 'scottish_gaelic', 'spanish', 'swedish', 'urdu_india', 'urdu_pakistan',                                 'welsh'
