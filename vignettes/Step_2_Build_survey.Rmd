...
+ `offline_mode` - Logical (`TRUE` or `FALSE`) specifying whether to upload input response data to Dropbox. This must be `TRUE` when uploading the package to the Shiny Server or data *will not be saved*. This is included so during the development stage, going back and forth through the survey will not save non-participant responses to the survey in your Dropbox App Folder.  
...
specify the language used on the BIO-WELL sliders. This can be selected from 32
different languages listed below. We recommend ensuring that the questions input are also in
this language.
...
![Your saved R script](https://raw.githubusercontent.com/r-a-dobson/BIO-WELL/main/man/V3/V3_0.png){width=75%}
